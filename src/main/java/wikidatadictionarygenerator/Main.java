...
public class Main {
    ...
    private static final int ZH_HANS = 2;
    private static final int ZH_HANT = 4;
    private static final int ZH_HK = 6;
    private static final int ZH_MO = 8;
    private static final int ZH_MY = 10;
    private static final int ZH_SG = 12;
    private static final int ZH_TW = 14;
    private static final int ZH_CN =16;
    private static final int ENGLISH = 18;
    private static final int DESCRIPTION = 19;// We seem to have two tabs between the last field and this

    // Note that these are in precedence order
    private static final int[] SIMPLIFIED_FIELDS = new int[] { ZH_HANS, ZH_CN, ZH_MY, ZH_SG };
...
//    private static final String INPUT_FILE = "intermediate_data/intermediate_after_excluding_stuff.tsv";
    private static final String INPUT_FILE = "intermediate_data/output_2024_dump_filtered.tsv";
    ...
    private static boolean ignoreRow(String[] segments) {
        ...
        if (IGNORE_ENTRIES_WITH_NO_NAME_OR_DESCRIPTION && segments[ENGLISH].isEmpty() && segments[DESCRIPTION].isEmpty()) {
            return true;
        }
        ...
        if (UNAMBIGUOUS_PINYIN_ONLY && (!HanUtils.pinyinIsUnambiguous(getSimplified(segments))
                || !HanUtils.pinyinIsUnambiguous(getTraditional(segments))) ) {
            return true;
        }
